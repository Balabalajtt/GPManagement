...
public class TeShenbaoAdapter extends RecyclerView.Adapter<TeShenbaoAdapter.ViewHolder> implements View.OnClickListener {

    private Context mContext;
    private List<SubjectData> mDatas;
    ...
    public TeShenbaoAdapter(Context context, List<SubjectData> datas) {
        mContext = context;
        mDatas = datas;
    }
    ...
    @NonNull
    @Override
    public TeShenbaoAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.recycler_view_te_shenbao, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull TeShenbaoAdapter.ViewHolder holder, int position) {
        holder.mTvName.setText("学生姓名：" + mDatas.get(position).studentName);
        holder.mTvTitle.setText("课题名称：" + mDatas.get(position).subjectName);
        holder.mTvType.setText("课题类型：" + mDatas.get(position).subjectType);
